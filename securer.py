...
ttlText = requests.get('https://restaurierungsvokabular.solidweb.org/annotations/annotations3.ttl').text

# check if the ttl file has changed
with open('comments3.ttl', 'r') as f:
    fileText = f.read()

if fileText != ttlText:
    with open('comments3.ttl', 'w') as f:
        f.write(ttlText)
